...
Shader "Slider Ramp/SliderRamp TREE"
{
	Properties
	{
	...
	[TCP2Separator]
		...
		_BendMinMax("Bend Min Max Height", Vector) = (0, 3, 0, 0)
		_BendAmount("Bend Amount", Range(0, 10)) = 1.0

		_BendDirection("Bend Direction", Vector) = (1, 0, 0, 0)
	...
	SubShader
	{
		...
		{
			float bendFactor = saturate((v.vertex.y - _BendMinMax.x) / (_BendMinMax.y - _BendMinMax.x));

			v.vertex.xyz += _BendDirection.xyz * (bendFactor * bendFactor * _BendAmount);
		}
